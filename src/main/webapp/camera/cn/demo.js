...
$(function () {
    ...
    // 初始化插件参数及插入插件
    WebVideoCtrl.I_InitPlugin(1650, 700, {
    ...
    $.getJSON("getCameraSet",function (data,status){
    	ip=data.cip;
    	port=data.cport;
    	username= data.cuser;
    	password=data.cpw;
...
function clickCapturePic() {
    ...
    if (oWndInfo != null) {
        ...
        if (0 == iRet) {
            szInfo = "抓图成功！地址D:\\动力系统\\Web Kit\\CaptureFiles";

			toastr["success"]("抓图成功，地址D:\\动力系统\\Web Kit\\CaptureFiles","成功");
        } else {

			toastr["error"]("截图成功","失败");
            szInfo = "抓图失败！";
        }
        showOPInfo(oWndInfo.szDeviceIdentify + " " + szInfo);
    }
...
function clickSsetting(){
	
}

window.onbeforeunload=clickStopRealPlay();
